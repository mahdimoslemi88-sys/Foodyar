...
export const analyzeRecipe = async (
  ...
  try {
    ...
    const recipeDetails = menuItem.recipe.map(r => {
      const ing = inventoryMap.get(r.ingredientId);
      return {
...
export const generateMenuEngineeringAnalysis = async (
  ...
  try {
    ...
    // Pre-calculate sales count per menu item
    const salesCountMap = new Map<string, number>();
    sales.forEach(sale => {
      sale.items.forEach(si => {
        salesCountMap.set(si.menuItemId, (salesCountMap.get(si.menuItemId) || 0) + si.quantity);
      });
    });
